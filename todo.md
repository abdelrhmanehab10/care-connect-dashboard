...
- [ ] Persist DataTable cell edits: add a `cell-edit-complete` handler to commit `event.newValue`/`event.newData` to `appointments` so inline edits are not discarded.
